...
- Adjusted the PR Agent Inbox workflow so waiting-only states publish `agent-inbox-clean` as pending without making the workflow check red, while agent-actionable items and publication failures still fail the automation.
