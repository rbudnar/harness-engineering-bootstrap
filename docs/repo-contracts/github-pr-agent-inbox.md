...
- Check suppression is an exact canonical workflow/job pair. `PR Agent Inbox / resolve-targets`, `PR Agent Inbox / agent-inbox`, and `PR Agent Inbox Signal / signal` are ignored; the same job under another workflow, or another job under the same workflow, remains observable. The scalar `agent-inbox-clean` commit status is ignored separately.
...
- Manual empty-input recovery enumerates at most 100 open PRs, includes drafts, deduplicates targets, and fails closed above the bound. Each queued job revalidates only its own target rather than recomputing the full fanout. Empty workflow-run PR associations use a bounded commit-to-open-PR fallback that also retains drafts.
