...
- Terminal-Bench lane: proved a one-task native Terminal-Bench smoke in WSL and recorded the blockers for a true HEB/no-HEB model-agent comparison.
...
Observed current package shape:
...
- In this environment, the executable entrypoint is `terminal-bench`; upstream docs may also refer to the CLI as `tb`.
...
Prompt-template adapter path:

- Terminal-Bench installed agents use `--agent-kwarg prompt_template=<path>` to render task instructions through a Jinja2 template containing `{{ instruction }}`.
- A true HEB/no-HEB comparison should run the same model agent, task ids, attempts, timeouts, and dataset version with two templates:
...
- Terminal-Bench `terminal-bench-core==0.1.1` can run from WSL here on one selected task: oracle resolved `swe-bench-langcodes`; nop did not.
- Windows-native Terminal-Bench execution has concrete encoding, Unix-tool, Git line-ending, and Docker path blockers.

Inference:

- Terminal-Bench is a viable external benchmark lane for this repo when run from WSL or Linux.
- HEB/no-HEB Terminal-Bench comparison should be implemented through prompt templates or a custom agent wrapper, not by changing Terminal-Bench tasks or scoring.
...
If a future decision needs public benchmark evidence, run a separate model-backed Terminal-Bench comparison from WSL/Linux with credentials available, fixed task ids, fixed model and agent surface, and two checked prompt templates. Keep those results separate from the local HEB pilot table.
