...
- MCP transport boundary (when MCP is used): treat MCP server capability/scope broadening (new tools or expanded data access) as an execution-surface change requiring re-approval. If the client launches STDIO servers from local config, also treat config and server registration/install changes (including STDIO command entries) as execution-surface changes; require explicit command/args allowlists (no shell), and run servers with least privilege in a sandbox where practical
