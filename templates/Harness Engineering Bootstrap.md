...
- MCP transport boundary (when MCP is used): if the client launches STDIO servers from local config, treat config, server registration/install changes (including STDIO command entries), and MCP server capability/scope broadening (new tools or expanded data access) as execution-surface changes; require explicit command/args allowlists (no shell), re-approval when tool surfaces expand, and run servers with least privilege in a sandbox where practical
