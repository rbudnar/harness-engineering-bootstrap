...
- OTel guidance when enabled: represent a session or task as a root span; use child spans for planning, model calls, tool calls, MCP requests, tests, retries, human approvals, and handoffs; include attributes for model, tool, route, decision/contract IDs, token/cost estimates, status, and failure category; prefer OTel GenAI semantic conventions (`gen_ai.*`) where supported, record the emitted semconv version, and document any stability opt-in or version change as a compatibility decision (for example, `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`)
...
### Routed Measurement Detail

Keep Phase 8 as the adoption spec. When a repository accepts deeper measurement, route implementation detail to [`templates/references/measurement-layer.md`](references/measurement-layer.md).

That reference covers:

- `scripts/harness-metrics.*` JSON shape and optional categories
- `scripts/harness-health.*` advisory/strict behavior and exit codes
- human correction markers for PR bodies, issue comments, inline review comments, and review submissions
- feedback-log shape for frequent agent iteration
- metrics storage and scheduled report destinations
- optional harness regression evals and prediction checks

Do not copy the whole reference into consuming repos. Pull only the smallest section that matches accepted trigger evidence. Keep GitHub/PR metrics, health reports, feedback logs, and evals absent until local history proves they are worth their cost.
